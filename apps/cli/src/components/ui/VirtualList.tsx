...
export function VirtualList<T>({
  ...
  useInput((input, key) => {
    ...
    if (key.pageUp) {
      ...
      const newIndex = Math.min(newOffset + terminalHeight - 1, data.length - 1)
      ...
      {showHeader && (
        <Box>
          <Text color="cyan" bold>
            Показано {scrollOffset + 1}-{Math.min(scrollOffset + terminalHeight, data.length)} из{' '}
            {data.length}
          </Text>
          {selectedIndex >= 0 && (
            <>
...
>({ item, index, isSelected }: RenderItemProps<T>) {
  return (
    <Box paddingX={1} paddingY={0}>
      ...
      <Text> </Text>
      <Text color={isSelected ? 'white' : undefined}>{item.name || `Item ${index + 1}`}</Text>
      <Text> </Text>
      {item.status && (
        <>
